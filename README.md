...
* Artifacts generated by the pipeline are stored in an Amazon S3 bucket called `ci-cd-pipeline-artifacts-{accountId}-{region}`.
* Artifacts which are specific for the application are stored in an Amazon S3 bucket called `books-api-artifacts-{accountId}-{region}`.
